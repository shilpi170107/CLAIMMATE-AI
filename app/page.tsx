...
export default function Page() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f7f9f6] text-[#10231c]">
      <nav className="fixed inset-x-0 top-0 z-40 border-b border-[#dbe8df]/80 bg-[#f7f9f6]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
          ...
          <div className="hidden items-center gap-8 text-sm font-medium text-[#52645b] md:flex">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setLanguage(language === 'EN' ? 'HI' : 'EN')} className="hidden rounded-full border border-[#d8e4dc] px-3 py-2 text-xs font-bold text-[#52645b] sm:block" aria-label="Change language">{language}</button>
        ...
        {menuOpen && <div className="flex flex-col gap-4 border-t border-[#dbe8df] px-5 py-5 text-sm font-medium md:hidden"><button onClick={() => scrollTo('solution')}>Solution</button><button onClick={() => scrollTo('journey')}>How it works</button><button onClick={() => scrollTo('impact')}>Impact</button><button onClick={() => setShowDemo(true)} className="rounded-full bg-[#123b2a] px-4 py-3 text-white">Try the interactive demo</button></div>}
      </nav>
      ...
      <section id="top" className="relative mx-auto grid max-w-7xl gap-14 px-5 pb-20 pt-36 lg:grid-cols-[1.02fr_.98fr] lg:px-8 lg:pb-28 lg:pt-48">
        <div className="relative z-10 max-w-2xl">
          ...
          <p className="mt-7 max-w-xl text-lg leading-8 text-[#617269]">ClaimMate turns confusing policies and stressful claims into a guided journey people can complete with confidence.</p>
          <div className="mt-9 flex flex-col gap-3 sm:flex-row"><button onClick={() => setShowDemo(true)} className="rounded-full bg-[#123b2a] px-6 py-3.5 text-sm font-semibold text-white shadow-lg shadow-[#123b2a]/15 transition hover:-translate-y-0.5 hover:bg-[#1c5740]">See how it works <ArrowRight className="ml-2 inline size-4" /></button><button onClick={() => scrollTo('solution')} className="rounded-full border border-[#cbdcd1] bg-white px-6 py-3.5 text-sm font-semibold text-[#244436]">Explore the solution</button></div>
        ...
        <div className="relative flex items-center justify-center lg:justify-end">
          ...
          <div className="relative w-full max-w-[470px] rounded-[2rem] border border-white/80 bg-[#e5f7e9] p-4 shadow-2xl shadow-[#174c34]/10 sm:p-6">
            <div className="rounded-[1.5rem] bg-white p-5 shadow-sm sm:p-7">
              ...
              <div className="mt-7 flex flex-col gap-3">{['Policy match confirmed', 'Hospital bill verified', 'Discharge summary pending'].map((item, i) => <div key={item} className="flex items-center gap-3 rounded-xl bg-[#f5f9f5] p-3 text-sm"><span className={`grid size-7 place-items-center rounded-full ${i === 2 ? 'bg-[#fff2d2] text-[#b4790d]' : 'bg-[#d9f6e0] text-[#179d53]'}`}>{i === 2 ? <Upload className="size-3.5" /> : <Check className="size-3.5" />}</span><span className="text-[#53645b]">{item}</span></div>)}</div>
            ...
            <div className="absolute -bottom-5 -left-5 flex items-center gap-3 rounded-2xl border border-[#d8eee0] bg-white px-4 py-3 shadow-xl"><span className="grid size-9 place-items-center rounded-xl bg-[#dff7e5] text-[#18a957]"><MessageCircle className="size-4" /></span><div><p className="text-xs text-[#74867b]">ClaimMate assistant</p><p className="text-sm font-semibold">Here when you need us</p></div></div>
          </div>
        </div>
      </section>

      <section className="border-y border-[#e0ebe3] bg-white/60"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-center gap-x-12 gap-y-4 px-5 py-6 text-center text-xs font-bold uppercase tracking-[.18em] text-[#8b9d92] lg:justify-between lg:px-8"><span>Health insurance</span><span>Fintech</span><span>Banking</span><span>Customer-first AI</span><span>Secure by design</span></div></section>
